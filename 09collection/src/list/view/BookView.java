...
public class BookView {
	...
	/**
	 * 전체 조회 - BookService에 있는 bookList를 얻어와 존재하는 모든 요소의 정보를 출력
	 */
	...
	/**
	 * 입력 받은 책 제목이 포함된 모든 책 출력하기 단, 없으면 "조회 결과가 없습니다" 출력
	 */
...
//		if(searchList.size() == 0) {
	...
	/**
	 * 검색할 가격의 범위(최소값, 최대값)을 입력 받아 가격 범위 내의 모든 책을 조회하여 출력 단, 없으면 "조회 결과가 없습니다" 출력
	 */
	...
	/**
	 * 제목, 글쓴이 가격을 입력 받아 BookService의 bookList에 추가 하기
	 */
	...
	/*
	 * 인덱스 번호를 입력 받아 1.일치하는 인덱스가 있으면 bookList에서 제거후 [책제목] 책이 목록에서 제거 되었습니다.
	 * 
	 * 2. 일치하는 인덱스가 없으면 "해당 인덱스에 존재하는 책이 없습니다"
	 */
	...
	private void updateBook() {
		...
		// 인덱스가 일치라는 요소의 BookDTO(참조 주소)를 반환 받기
		
		//ex) index가 0인 경우 
		// BookSerivice의 bookList.get(0)에 저장된 주소와 
		// 아래 book에 저장된 주소가 "같음"
		BookDTO book = service.seletIndex(index);
		...
		if(book == null) {
			...
			//bookㅇ; 참조하는 BookDTO의 가격을 newPrice로 수정
